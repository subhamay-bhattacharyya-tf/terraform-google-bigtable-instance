...
This is a **Terraform module** that creates and manages a single `google_bigtable_instance` resource on GCP. The entire public interface is one input variable (`bigtable_config`) and five outputs (id, name, project, instance_type, deletion_protection). This will be used as a GitHub Repository Template. The actual modules with me implemented seperately.
...
cd test && go test -v -timeout 30m -run TestBigtableBasic ./bigtable_basic_test.go ./helpers_test.go
...
├── main.tf            # Single google_bigtable_instance resource
├── variables.tf       # bigtable_config object variable with all validations
├── outputs.tf         # Five bigtable instance attribute outputs
├── versions.tf        # Terraform >= 1.3.0, google provider >= 7.23.0
├── examples/
│   └── bigtable/basic/  # Reference usage; CI validates this separately
└── test/
    ├── bigtable_basic_test.go   # Terratest: creates real bigtable instance, asserts outputs, destroys
...
The module uses a single structured `bigtable_config` object rather than flat variables. All validation (naming rules, instance type enum, project ID format, deletion protection values) lives in `variables.tf`.
